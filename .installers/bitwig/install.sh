# Based on: https://github.com/teervo/bitwig-fedora
...
function check_if_already_built() {
    ...
    if [ -f $rpm ]; then
        echo RPM package already built
   		echo Install using sudo zypper install $rpm
        exit 0
    fi
}
...
function build_rpm() {
  ...
  arch=$(uname -m)
	...
	RPM_FILE=rpmbuild/RPMS/$arch/$(rpm_basename) &&
	mv $RPM_FILE $PWD &&
	echo &&
	echo RPM created. &&
	echo Install using sudo zypper install $(basename $RPM_FILE)
}
